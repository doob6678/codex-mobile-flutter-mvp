...
- 2026-06-05: `flutter analyze` passed. `flutter test test\model_api_test.dart test\app_widget_test.dart` passed with 57 tests. `flutter build apk --release` passed and produced `mobile_app\build\app\outputs\flutter-apk\app-release.apk`. `flutter build web --release --base-href /ipad/ --pwa-strategy none` passed and the output was copied into `dist\mobile-ipad-web`, `dist\bridge-windows\wwwroot\ipad`, and `dist\bridge-framework-dependent\wwwroot\ipad`.
- 2026-06-05: Added a widget regression for tapping a conversation `00-总目录.md` file link: it opens `FilePreviewScreen`, renders Markdown via `MarkdownBody`, and duplicate taps during a held `readFile` do not issue a second request. Re-ran `flutter analyze`, `flutter test test\model_api_test.dart test\app_widget_test.dart` with 58 passing tests, `flutter build apk --release`, and `flutter build web --release --base-href /ipad/ --pwa-strategy none`.
